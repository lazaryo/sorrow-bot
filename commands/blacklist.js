exports.run = (bot, message, args, about, rn, sorrows, displayWords, checkWord, singleWord, prefix, botUptime, banned, checkID, fs, newGuildHook, blacklistHook, safe) => {
    ...
    let serverToBlacklist = message.content.split(` `)[2];
...
//    } else {
//        if (banned.blacklist.includes(serverToBlacklist) == true) {
//            for (let guild of bot.guilds) {
//                guild = guild[1];
//                guild.includes(serverToBlacklist);
//                message.channel.send(`This guild has already been blacklisted. Leaving the guild (${guild.name}) now.`);
//                return guild.leave();
//            }
//            return message.channel.send('This server has already been blacklisted.');
//        }
//        
//        if (safe.whitelist.includes(serverToBlacklist)) {
